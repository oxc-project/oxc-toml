...
impl Context {
    ...
    /// This is a no-op now that scoped options have been removed.
    fn update_options(&self, _opts: &mut Options, _range: TextRange) {
        // No-op: scoped options removed
    }
...
fn dangling_newlines(t: SyntaxToken) -> Option<usize> {
    ...
    if let Some(nt) = t.next_sibling_or_token()
        && let Some(nnt) = nt.next_sibling_or_token()
            && nt.kind() == WHITESPACE && nnt.kind() == NEWLINE {
